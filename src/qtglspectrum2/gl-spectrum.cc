...
#define BAR_SPACING (1.2f / NUM_BANDS)
...
public:
    static constexpr PluginInfo info = {
        N_("OpenGL Spectrum Analyzer - Circle"),
        PACKAGE,
        gl_about,
        nullptr, // prefs
        PluginQtOnly
...
class GLSpectrumWidget : public QOpenGLWidget, protected QOpenGLFunctions_1_0
{
public:
...
void GLSpectrumWidget::draw_bars ()
{
    ...
    float d{0.0}, x{0.0}, z{0.0}, r{1.5};
    constexpr float div{3.14/180};
    for (int i = 0; i < NUM_BANDS; i ++)
    {
        for (int j = 0; j < NUM_BANDS; j ++)
        {
            x = r * cosf(d * div), z = r * sinf(d * div);
            draw_bar (x, z,
